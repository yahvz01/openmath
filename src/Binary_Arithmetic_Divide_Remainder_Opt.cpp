...
std::pair<int16_t, int16_t> openmath::divide_remainder_ptr(int8_t* const dividend, const int16_t& dividend_byte_length, int8_t* const divisor, const int16_t& divisor_byte_length, int8_t** result_qutient, int8_t** result_remainder)
{
    ...
    //bool is_positive_of_dividend( true);
    //bool is_positive_of_divisor( true);
    ...
    {
        int8_t* temp_swap(make_binary_complement_ptr(add_element_for_process, dividend_byte_length));
        free(add_element_for_process);
        add_element_for_process = temp_swap;
    }
    ...
    std::pair<int16_t, int16_t> result_bit_lengths(0, 0);
    ...
    if(!is_positive_of_dividend)
    {
        *result_remainder = make_binary_complement_ptr(remainder_ptr, dividend_byte_length);
    }
    else
    {
        *result_remainder = static_cast<int8_t*>(malloc(dividend_byte_length));
        memcpy(*result_remainder, remainder_ptr, dividend_byte_length);
    }
    result_bit_lengths.second = remainder_bit_length;
    
    // qutient return process 
    ...
    if(is_positive_of_dividend != is_positive_of_divisor)
    {
        ...
        *result_qutient = shrink_unlimitedint_memory_and_free(complemented_qutient, dividend_byte_length, qutient_byte_length);
    }
    else
    {
        *result_qutient = shrink_unlimitedint_memory(temp_result, dividend_byte_length, qutient_byte_length);
    }
    result_bit_lengths.first = qutient_bit_length;
    free(temp_result);
    return result_bit_lengths;
}
